...
Run multi-table PLS-C using rsfMRI power data and MEG power data
    A .01 - .1 Hz bandpass filter is applied to timeseries
...
def _extract_average_power_bp(hdf5_file, sessions, subjects, rois, image_type):
    """
    ...
    for sess in sessions:
        ...
        for subj in subjects:
            ...
            fs = 1/.72
            cutoffs = [.01, .1]
            filtered = butter_filter(data, fs, cutoffs)

            fft_power = np.absolute(np.fft.rfft(filtered, axis=0))**2
...
if __name__ == "__main__":
    ...
    mri_data = _extract_average_power_bp(mri_path, mri_sess, all_subj, rois, 'MRI')
    meg_data = _extract_average_power_bp(meg_path, meg_sess, all_subj, rois, 'MEG')
