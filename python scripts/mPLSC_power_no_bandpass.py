# -*- coding: utf-8 -*-
"""
Run multi-table PLS-C using rsfMRI power data (no bandpass filter)
    and MEG power data (resampled to rsfMRI sampling rate)
...
def _extract_average_power(hdf5_file, sessions, subjects, rois, image_type):
    """
    ...
    In this function, MRI data has not been bandpass-filtered, and MEG data
    has been downsampled to the sampling rate of the MRI data
    """
    power_data = {}
...
if __name__ == "__main__":
    ...
    mri_data = _extract_average_power(mri_path, mri_sess, all_subj, rois, 'MRI')
#     meg_data = _extract_average_power(meg_path, meg_sess, all_subj, rois, 'MEG')
#     x_tables = _merge_mri_meg(mri_data, meg_data)
    x_tables = [mri_data[t].values for t in list(mri_data)]
    ...
    x_table_names = mri_subtable_names + meg_subtable_names
    ...
    fig_path = pdir + '/figures/mPLSC_power_no_bandpass'
    ...
    with open(pdir + '/data/mPLSC_power_no_bandpass.pkl', 'wb') as file:
        pkl.dump(output, file)
        
#    with open(pdir + '/data/mPLSC_power_no_bandpass.pkl', 'rb') as file:
