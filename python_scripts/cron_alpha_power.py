...
def plot_dist(cron_alpha_values, fname=None, nbins=20):
    mu = np.mean(cron_alpha_values)
    sigma = np.std(cron_alpha_values, ddof=1)

    sns.set()
    plt.clf()
    fig, ax = plt.subplots(figsize=(12, 8))
    n, bins, _ = ax.hist(cron_alpha_values, bins=nbins, density=False)
    y = norm.pdf(bins, mu, sigma)
    # est = kde.gaussian_kde(cron_alpha_values)
    # ax.plot(bins, y, '--')
    # ax.set_yticklabels(np.arange(0, 3, 3))
    # ax.set_yticklabels(np.linspace(0, 1, 5))
    ax.set_ylabel('Frequency', fontsize='x-large')
    ax.set_xlabel("Cronbach's alpha", fontsize='x-large')
...
if __name__ == "__main__":
    ...
    mri_crons.to_excel(fig_path+'/cron_alpha_nyquist_mri.xlsx')
    meg_crons.to_excel(fig_path+'/cron_alpha_nyquist_meg.xlsx')

    print('%s: Plotting cron alpha distributions' % pu.ctime())
    plot_dist(mri_crons.values, fname=fig_path+'/hist_nyquist_mri.png')
    plot_dist(meg_crons.values, fname=fig_path+'/hist_nyquist_meg.png')

    print('%s: Running conjuntion between modes' % pu.ctime())
    conj = _conjunction(mri_crons, meg_crons, out_type='full')
    if conj is not None:
        conj.to_excel(fig_path+'/conjunction_nyquist_mri_and_meg.xlsx')
        ...
        fname = fig_path + '/conjunction_nyquist_mri_and_meg.png'
    ...
    print('%s: Saving results' % pu.ctime())
    mri_crons.to_excel(fig_path+'/bandpass_mri_cron_alpha.xlsx')
    meg_crons.to_excel(fig_path+'/bandpass_meg_cron_alpha.xlsx')

    print('%s: Plotting cron alpha distributions' % pu.ctime())
    plot_dist(mri_crons.values, fname=fig_path+'/hist_bandpass_mri.png')
    plot_dist(meg_crons.values, fname=fig_path+'/hist_bandpass_meg.png')

    print('%s: Running conjuntion between modes' % pu.ctime())
    conj = _conjunction(mri_crons, meg_crons, out_type='full')
    if conj is not None:
        conj.to_excel(fig_path+'/conjunction_bandpass_mri_and_meg.xlsx')
        ...
        # rois_to_stack = conj.index
        # custom_roi = mf.create_custom_roi(roi_path, rois_to_stack, mags)
        # fname = fig_path + '/conjunction_bandpass_mri_and_meg.png'
        # fig = mf.plot_brain_saliences(custom_roi, minval=None, figpath=fname)
